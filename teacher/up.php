  ...
  if (preg_match("/TOTAL/i", $allDataInSheet[3][$highestColumm]))
{
  ...
  for($i=4;$i<= $highestRow;$i++){
    $t_student++;
    if((is_numeric($allDataInSheet[$i][$highestColumm]))&&($allDataInSheet[$i][$highestColumm] != 0)){

      if(($allDataInSheet[$i][$highestColumm] < $pass_mark)&&($allDataInSheet[$i][$highestColumm] != 0)){$no_fail++;}else{$no_pass++;}
    $c_sum = $c_sum+$allDataInSheet[$i][$highestColumm];
    $pres++;
  } if($allDataInSheet[$i][$highestColumm]==0 ){$absent++;}
  }
  $c_avg=($c_sum/$pres);
...
function finalcalc($marray,$k,$allDataInSheet)
{
    ...
    $A_val=$B_val=$C_val=$D_val=$E_val=$F_val=0;
	for($i=1;$i<=$k;$i++){
		     if($marray[$i][2]=='A' || $marray[$i][2]=='a'){
                    $count_A=$count_A+1;


                    $A_val=($A_val+$marray[$i][8]);
		     }
		     elseif($marray[$i][2]=='B' || $marray[$i][2]=='b'){
                    $count_B=$count_B+1;
                    $B_val=$B_val+$marray[$i][8];
		     }
		     elseif($marray[$i][2]=='C'||$marray[$i][2]=='c' ){
                    $count_C=$count_C+1;
                    $C_val=$C_val+$marray[$i][8];
		     }
		     elseif($marray[$i][2]=='D'||$marray[$i][2]=='d' ){
                    $count_D=$count_D+1;
                    $D_val=$D_val+$marray[$i][8];
		     }
		     elseif($marray[$i][2]=='E'||$marray[$i][2]=='e' ){
                    $count_E=$count_E+1;
                    $E_val=$E_val+$marray[$i][8];
		     }
         elseif($marray[$i][2]=='F'||$marray[$i][2]=='f' ){
                    $count_F=$count_F+1;
                    $F_val=$F_val+$marray[$i][8];
         }
       }
       //echo $count_A;
      /* $A_val=round($A_val,2);
...
//$cid=$_SESSION['cid'];
 ...
 $tname=trim($allDataInSheet[1]["A"]);
       ...
       mysql_query("insert into testresult (cid,bid,sid,subid,testname,co1,co2,co3,co4,co5,co6,count_co1,count_co2,count_co3,count_co4,count_co5,count_co6) values('$cid','$bid','$sid','$subid','$tname','$A_val','$B_val','$C_val','$D_val','$E_val','$F_val','$count_A','$count_B','$count_C','$count_D','$count_E','$count_F')") or die(mysql_error());
   modtable($count_A, $count_B, $count_C,$count_D ,$count_E,$count_F,$A_val,$B_val,$C_val,$D_val,$E_val,$F_val)  ;  
}

function modtable($count_A, $count_B, $count_C,$count_D ,$count_E,$count_F,$A_val,$B_val,$C_val,$D_val,$E_val,$F_val) 
{
 $c_id=$_SESSION['cid'];
 $b_id=$_SESSION['bid'];
 $s_id=$_SESSION['sid'];
...
//echo $sub_id;
$result = mysql_query("SELECT cid,bid,sid,no_test,co1,co2,co3,co4,co5,co6,count1,count2,count3,count4,count5,count6 FROM finalresult WHERE subid='$sub_id'  AND bid='$b_id'");
  ...
  //echo $no_test;
   ...
   //	echo $co11;
    ...
    $count6=$row[15];
     ...
     $count6=$count6+$count_F;

   	   
   	
   
   	 mysql_query("UPDATE finalresult SET no_test='$no1_test', co1='$co11',co2='$co21',co3='$co31',co4='$co41',co5='$co51',co6='$co61',count1='$count1',count2='$count2',count3='$count3',count4='$count4',count5='$count5',count6='$count6' where subid='$sub_id'  AND bid='$b_id'") or die(mysql_error());
    ...
    // no data matched
                   ...
                   // echo $no;
                    ...
                    mysql_query("insert into finalresult (cid,bid,sid,subid,no_test,co1,co2,co3,co4,co5,co6,count1,count2,count3,count4,count5,count6) values('$c_id','$b_id','$s_id','$sub_id','$no','$A_val','$B_val','$C_val','$D_val','$E_val','$F_val','$count_A','$count_B','$count_C','$count_D','$count_E','$count_F')") or die(mysql_error());
}
