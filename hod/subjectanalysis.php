...
$s1=$ss1=$sss1=$s2=$ss2=$sss2=$s3=$ss3=$sss3=$s4=$ss4=$sss4=$s5=$ss5=$sss5=$s6=$ss6=$sss6=$cec=$sum1=$sum2=$uni=$un=$u=$ol=$ol1=$out=$op=$op1='0';
...
$result = mysqli_query($conn, $sql);
 ...
 $sqlmn="SELECT * FROM overall WHERE subid= '$subid' AND bid='$bid' "; //selection query
                   $rsmn = mysqli_query($conn, $sqlmn);
            ...
            while($row = mysqli_fetch_assoc($result4)) {
               ...
               // echo $row["count1"];
                ...
                $sem=$row["semester"];
                  ...
                  //odbc_exec($conn,$sql);
                   if (mysqli_num_rows($rsmn) > 0) {
    // output data of each row

                   while($row = mysqli_fetch_assoc($rsmn)) {
                   $fname=$row['fname'];
                   
    }
}else{$fname='No Faculty Enroled';}
...
//mysqli_close($conn);
if (mysqli_num_rows($resultove) == 0 && ($uni!=0)){
